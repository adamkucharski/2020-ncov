...
*Note: this is preliminary analysis and has not been peer reviewed*

## Summary

• **To estimate the early dynamics of transmission in Wuhan, we fitted a mathematical outbreak model to multiple available datasets** on international exported cases from Wuhan and cases in Wuhan. Fitting to multiple data sources rather than a single dataset (or data point) is particularly useful for estimates real-time, because some datasets may be unreliable. 

• **Transmission was a random process in the model, and could vary over time -- this means the model can uncover fluctuations in transmission** that might be the result of increased transmission or reductions from control measures. Our group previously used a similar analysis to understand the dynamics of [Ebola in Liberia](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5394640/)

• **We assumed that the chance of cases being exported from Wuhan to other countries depended on the number of cases in Wuhan, the number of outbound travellers (accounting for travel restrictions after 23rd January), and the [relative connectivity of different countries](https://www.mobs-lab.org/2019ncov.html)**. We considered the 30 countries outside China most at risk of exported cases in the analysis. The model accounts for delays in symptom onset and reported (see methods below).
...
• The estimated peak in R0 in early Jan (Figure 1B) is the risk of a sudden rise in cases around 16th January both in Wuhan (Figure 1C) and internationally (Figure 1F).
...
```{r fig_schematic, echo=FALSE, fig.align='center', fig.cap="_Figure 2: Model schematic. The population is divided into susceptible-exposed-infectious-removed, with a fraction of exposed individuals subsequently travelling and eventually being detected in their destination country_", out.width = '40%'}
knitr::include_graphics("../plots/schematic.png")
```

#### Fitting

We estimated the time varying reproduction using sequential Monte Carlo by jointly fitting to three datasets:
...
1. Daily incidence of early cases in China, by date of onset. We assume that these are all in Wuhan.  The most recent two data points are omitted during fitting as they are likely to be strongly influenced by delays in reporting. Source: [Liu et al](https://www.biorxiv.org/content/10.1101/2020.01.25.919787v1), [HKU](https://www.med.hku.hk/f/news/3549/7418/Wuhan-coronavirus-outbreak_AN-UPDATE_20200127.pdf)
...
We used a Poisson observation model with different relative reporting probability for cases inside and outside of Wuhan, and only a proportion of cases having known onset dates. 

Plots were generated by running 200 repetitions of SMC with 1000 particles. The volatility and reporting parameters were selected based on a grid search to find the maximum likelihood. We assumed the outbreak started on 15th November with 1 infectious individual and 1 in their incubation period. We also assume all infectious people eventually become symptomatic and would be eventually be detected in destination country if they travelled by plane. We assumed that the population at risk was 10m (rather than 19m assumed in Imperial analysis) because most cases to date are adults and this group are more likely to travel. Model is available here: https://github.com/adamkucharski/2020-ncov
